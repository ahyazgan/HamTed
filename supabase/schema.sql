...
-- 'saha_hatti': vitrin ilanlarinda gosterilen YUKLET saha hatti numarasi. Firmanin
-- kendi numarasi DEGIL, platformun hattidir — herkese acik olmasi gerekir, yoksa
-- sahipsiz vitrin ilaninda aranacak numara hic gorunmez.
...
-- ──────────────────────────────────────────────
-- 17) SAHA PAKETI (migration-2026-08-saha-paketi.sql govdesi)
--    Bu blok schema.sql'de de DURMALI: taze bir veritabani yalniz bu dosyayla
--    kurulunca musaitlik/talep sinyali/duzenli sevkiyat/davet SESSIZCE eksik
--    kalirdi (2026-07 denetiminin en kritik bulgusunun aynisi).
--      a) profiles.available_until  : "Aracim bugun musait" damgasi
--      b) search_signals            : sonucsuz arama = talep sinyali
--      c) listings.recurring_*      : duzenli sevkiyat + onceki nakliyeciye oncelik
--      d) profiles.invite_code/invited_by : davet / referans zinciri
--    accept_job'in ONCELIK guard'i yukaridaki 7f blogunda DEGIL burada,
--    dosyanin sonunda tanimlanir (son create or replace kazanir).
-- ──────────────────────────────────────────────
...
-- b) TALEP SINYALI (yazma herkese — kayitsiz ziyaretcinin aramasi en degerli
--    sinyaldir; okuma yalniz admin)
...
-- c) DUZENLI SEVKIYAT
alter table public.listings add column if not exists recurring_freq     text;
...
declare
  t public.listings; v_iv interval; v_acik boolean; v_prev uuid; v_next timestamptz; v_count int := 0;
begin
  for t in
    select * from public.listings
     where recurring is true and parent_id is null
  ...
  loop
    ...
    select min(x) into v_next
      from generate_series(t.next_run_at, now() + v_iv, v_iv) g(x) where x > now();
    update public.listings set next_run_at = coalesce(v_next, now() + v_iv) where id = t.id;
    -- Sablonun kendisi ya da bir kopyasi hala acikken yeni sefer ACILMAZ.
    v_acik := (t.status = 'aktif') or exists (
      select 1 from public.listings c where c.parent_id = t.id and c.status = 'aktif');
    if v_acik then continue; end if;
    select c.accepted_by_id into v_prev from public.listings c
...
-- accept_job SON HALI: 7f blogundaki tum guard'lar + oncelik penceresi.
create or replace function public.accept_job(
  p_listing_id bigint, p_price numeric default null, p_vehicle jsonb default null
) returns public.listings language plpgsql security definer set search_path = public as $$
declare
...
begin
  if v_uid is null then raise exception 'Giriş gerekli.'; end if;
  select coalesce(name,''), coalesce(status,'aktif'), coalesce(tasima_turu,'')
    into v_name, v_status, v_turu from public.profiles where id = v_uid;
  if v_status = 'banli' then raise exception 'Hesabın askıya alındı.'; end if;
  select * into v_listing from public.listings where id = p_listing_id for update;
  if not found then raise exception 'İlan bulunamadı.'; end if;
  if v_listing.owner_id is null then raise exception 'Bu bir tanıtım ilanıdır — kabul edilemez.'; end if;
  ...
  if v_listing.reserved_for_id is not null and v_listing.reserved_until is not null
     and v_listing.reserved_until > now() and v_listing.reserved_for_id <> v_uid then
...
-- Karisan harf/rakam yok (0/O, 1/I/L): kod telefonda okunacak.
...
-- ──────────────────────────────────────────────
-- 18) SAHA ADAY KAYDI (migration-2026-08-saha-aday.sql govdesi)
--    Bu blok schema.sql'de de DURMALI: taze bir veritabani yalniz bu dosyayla
--    kurulunca saha onboarding SESSIZCE eksik kalirdi.
--
--    MODEL: once deger, sonra hesap. profiles.id -> auth.users(id) FK oldugu
--    icin hesapsiz profil acilamaz; bu yuzden saha ekibi ADAY KAYIT acar
--    (auth hesabi YOK), vitrin ilanlari owner_id NULL + prospect_id ile yayinlanir,
--    firma davet linkiyle kendi hesabini acinca claim_prospect hepsini devreder.
--
--    NOT: guard_driver_listing_update'in SAHIPLENME istisnasi yukaridaki 7. blokta
--    DEGIL burada, dosyanin sonunda tanimlanir (son create or replace kazanir) —
--    accept_job'in oncelik guard'iyla ayni kalip.
-- ──────────────────────────────────────────────

-- a) ADAY KAYIT — auth.users'a bagli DEGIL (kilidi acan sey bu).
--    phone/email/note YALNIZ admin okur; hicbir goruntuye/anon RPC'ye sizmaz.
create table if not exists public.prospects (
  id           bigint generated by default as identity primary key,
  token        text not null unique default (public.gen_invite_code() || public.gen_invite_code()),
  name         text not null,
  role         text not null default '',
  phone        text not null default '',   -- SAHA NOTU: yayinlanmaz
  ...
  -- RIZA KAPISI: OCAK-LISTESI.md'nin "izinsiz ilan olmaz" kurali artik belge
  -- tavsiyesi degil VERITABANI KISITI. Panelde dugmeyi gizlemek yetmez.
...
-- b) VITRIN ILANI BAGI. Vitrin ilani = owner_id NULL + prospect_id dolu.
--    owner_id'nin null kalmasi bilincli: enforce_owner_snapshot dokunmaz,
--    accept_job "tanitim ilanidir" diye reddeder, istemcide isShowcase dali kurulu.
...
-- c) SURUCU GUARD'I — yukaridaki surumun uzerine SAHIPLENME istisnasi eklenmis hali.
--    SART: claim_prospect owner_* kolonlarini yazar ama cagiran ne ilan sahibi
--    (owner_id null) ne admin. SECURITY DEFINER RLS'i atlar, TRIGGER'I ATLAMAZ —
--    bu istisna olmadan sahiplenme her seferinde patlar (ayni tuzak daha once
--    is kabulunu kirmisti: migration-2026-07-kabul-guard-fix.sql).
create or replace function public.guard_driver_listing_update()
returns trigger language plpgsql security definer set search_path = public as $$
declare
  allowed text[] := array['phase','status','cycle_stage','arrived_at','trips_done','delivery_proof','payment_received_at','payment_paid_at'];
begin
  ...
  -- DOGRUDAN KABUL gecisi (aktif -> eslesti)
  if old.status = 'aktif' and new.status = 'eslesti'
     and old.accepted_by_id is null and new.accepted_by_id = auth.uid() then
    allowed := allowed || array['accepted_by_id','assigned_vehicle'];
  end if;
  -- IPTAL gecisi (eslesti -> aktif)
  if old.status = 'eslesti' and new.status = 'aktif'
     and old.accepted_by_id = auth.uid() and new.accepted_by_id is null then
    allowed := allowed || array['accepted_by_id','assigned_vehicle'];
  end if;
...
-- f) SAHIPLENME: aday kaydi + vitrin ilanlari cagiranin hesabina gecer.
...
begin
  ...
  -- Tek kez sahiplenilir; ayni kisi tekrar tiklarsa ok:true (akis sessizce devam etsin).
  if p.claimed_by is not null then
  ...
  -- DEVIR KAPISI: yalniz TAZE hesap sahiplenebilir. Davet linki WhatsApp'ta
  -- iletilir ve gruplarda dolasir; bu kapi olmadan linki goren HERHANGI bir uye
  -- firmanin kaydini ustune alir (ilanlari devralir, adi firma adiyla ezilir).
  ...
  -- veriyor — ayni kurali burada da uygula (guard sessizce geri almasin).
  ...
  -- owner_verified/owner_rating ELLE yazilir: enforce_owner_snapshot yalniz
  -- BEFORE INSERT'te calisir, UPDATE'te rozeti tazelemez.
